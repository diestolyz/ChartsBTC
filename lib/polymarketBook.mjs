...
/**
 * @typedef {{ assetId: string; price: number; size: number; side: "BUY" | "SELL" | null; tsMs: number | null }} PmTradeEvent
 */

/**
 * @param {{ onTrade?: (ev: PmTradeEvent) => void } | void} [opts]
 */
export function createBookState(opts) {
  const onTrade = opts && typeof opts.onTrade === "function" ? opts.onTrade : null;
  ...
  function handleOneMessage(msg) {
    ...
    if (t === "last_trade_price") {
      ...
      if (onTrade && price != null && price > 0) {
        const size = msg.size != null ? pmNum(msg.size) : null;
        if (size != null && size > 0) {
          const sideRaw = msg.side != null ? String(msg.side).trim().toUpperCase() : "";
          /** @type {"BUY" | "SELL" | null} */
          const side = sideRaw === "BUY" ? "BUY" : sideRaw === "SELL" ? "SELL" : null;
          const T = msg.timestamp != null ? Number(msg.timestamp) : NaN;
          const tsMs = Number.isFinite(T) && T > 0 ? (T < 1e12 ? T * 1000 : T) : null;
          onTrade({ assetId: aid, price, size, side, tsMs });
        }
      }
    }
  }
